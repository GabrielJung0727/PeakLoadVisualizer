...
tagsWrap === null || tagsWrap === void 0 ? void 0 : tagsWrap.appendChild(statusTag);
let cpuChart;
let netChart;
let ws = null;
let lastLogTs = 0;
...
function pushVizArrow(type, severity) {
    if (!viz)
        return;
    const count = severity === 'error' ? 10 : severity === 'warn' ? 6 : 3;
    for (let i = 0; i < count; i += 1) {
        const arrow = document.createElement('div');
        const typeClass = type === 'ddos' ? 'ddos' : type === 'bruteforce' ? 'bruteforce' : type === 'portscan' ? 'portscan' : type === 'sqlinj' ? 'sqlinj' : '';
...
function pushPoint(chart, label, values) {
    ...
    chart.data.datasets.forEach((d, idx) => d.data.push(values[idx]));
...
function handleMetrics(evt) {
    ...
    if (stageTag)
        stageTag.textContent = `Stage: ${level}`;
    const warns = [];
    if (cpu > 85)
        warns.push('CPU High');
    if (netIn > 800)
        warns.push('Inbound Flood');
    updateTags(warns);
}
...
function updateButtonStates() {
    document.querySelectorAll('[data-action]').forEach((btn) => {
        const action = btn.dataset.action || '';
        const isActive = (action === 'ddos-start' && activeActions.has('ddos')) ||
